...
void MainWindow::sizeLogo()
{
    qDebug() << "Sizing logo";
    QPixmap pm;
    pm.load("/home/dave/MusicalPi/MusicalPi.gif");
...
//    if(nowMode!=playMode || !playing) return;  // Only pay attention in playmode-playing
    if(nowMode!=playMode&&nowMode!=libraryMode) return;  // Only pay attention in playmode-playing
    switch (nowMode)
    {
        case playMode:
            if(playing){
