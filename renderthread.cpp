...
#include <cmath>
//#include "/home/dave/poppler/qt5/src/poppler-qt5.h"
...
//        double scaleY = (double)mHeight / ((double)thisPageSize.height() / (double)72.0);
        double scaleX = static_cast<double>(mWidth) / (static_cast<double>(thisPageSize.width()) / static_cast<double>(72.0));
        double scaleY = static_cast<double>(mHeight) / (static_cast<double>(thisPageSize.height()) / static_cast<double>(72.0));
//        double desiredScale = std::trunc(std::min(scaleX, scaleY));  // For notational scores integers seem to give better alignment, sometimes.
        ...
        ourParent->document->setRenderHint(Poppler::Document::ThinLineShape,true);
#endif
