...
//    kbd = mParent->kbd;
    ...
    m_db = nullptr;  // Be sure we know if it's been created.  We open/close it a lot, but do not delete it until this routine is deleted
...
void musicLibrary::loadPlayLists()
{
    ...
    // SELECT t.name as tag_name, count(1) as count
    // FROM tags t, books_tags_link btl
    // where t.id=btl.tag
    // and t.name<>'SheetMusic'
    // and t.name not like 'musicList_' || '%'
    // group by t.name
        ...
        (
           ActiveListIndex == 0 ? "" :
              ( "inner join books_tags_link btl7 on btl7.book = b.id "
                "inner join tags t7 on t7.id = btl7.tag and t7.name = '" + calibreListPrefix + ActiveList + "' "
              )
        ) +
