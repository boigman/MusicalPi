...
MainWindow::MainWindow() : QMainWindow()
{
    ...
    timer = new QTimer(this); // instantiate timer....make timer child of the MainWindow class
    ...
    connect(timer, SIGNAL(timeout()), this, SLOT(readInputPins())); //connect timer's timeout() signal to the readInputPins() slot method
    timer->start(250); // start timer. The timeout() signal is emitted every 250ms....therefor readInputPins slot method() runs once every 250ms
#endif
...
{
   qDebug()<< "In MainWindow::setupPins";
   myPi = pigpio_start(NULL, NULL);
   if (myPi<0) exit(1);
   set_mode(myPi, 17, PI_INPUT);  // Set GPIO17 as input.
   set_mode(myPi, 18, PI_INPUT);  // Set GPIO18 as input.
   set_mode(myPi, 15, PI_INPUT);  // Set GPIO15 as input.
   set_mode(myPi, 14, PI_INPUT);  // Set GPIO14 as input.
   set_pull_up_down(myPi, 17, PI_PUD_DOWN);   // Sets a pull-down.
   set_pull_up_down(myPi, 18, PI_PUD_DOWN);   // Sets a pull-down.
   set_pull_up_down(myPi, 15, PI_PUD_DOWN);   // Sets a pull-down.
   set_pull_up_down(myPi, 14, PI_PUD_DOWN);   // Sets a pull-down.
}

void MainWindow::readInputPins(void){
    if(nowMode!=playMode) return;  // Only pay attention in playmode
    statePU = gpio_read(myPi, 17) ;
    if(statePU != statePrvPU) { 
       if(statePU ==1 || statePrvPU == -1) { 
         qDebug()<< "Pin PgUp State: "<< statePU << "\n";
         navigateTo(1);
       }
       statePrvPU = statePU;
    }
    stateL = gpio_read(myPi, 18) ;
    if(stateL != statePrvL) { 
       if(stateL ==1 || statePrvL == -1) { 
         qDebug()<< "Pin Left State: "<< stateL << "\n";
         navigateTo(leftmostPage - (pagesNowAcross * pagesNowDown));
       }
       statePrvL = stateL;
    }
    stateR = gpio_read(myPi, 15) ;
    if(stateR != statePrvR) { 
       if(stateR ==1 || statePrvR == -1) { 
         qDebug()<< "Pin Right State: "<< stateR << "\n";
         navigateTo(leftmostPage + (pagesNowAcross * pagesNowDown));
       }
       statePrvR = stateR;
    }
    statePD = gpio_read(myPi, 14) ;
    if(statePD != statePrvPD) { 
       if(statePD ==1 || statePrvPD == -1) { 
         qDebug()<< "Pin PgDn State: "<< statePD << "\n"; 
         navigateTo(MUSICALPI_MAXPAGES);
       }
       statePrvPD = statePD;
    }
}
...
//    int xheight = this->geometry().height();
//    int xwidth = this->geometry().width();
    qDebug() << "window height: " << QString::number(xheight) << ", width: " << QString::number(xwidth);
    qDebug() << this->geometry().height() << this->geometry().width();
    cornerleft.setParent(this);
    cornerleft.setName("Left");
    cornerleft.setKeycode(Qt::Key_Left);
    int cornermargin = 320;
    cornerleft.move(0,xheight-cornermargin);
    cornerright.setParent(this);
    cornerright.setName("Right");
    cornerright.setKeycode(Qt::Key_Right);
...
//    next2Button      = new QPushButton("Next2");
//    playerMenuLayout->addWidget(next2Button);
//    next2Button->move(500,500);
...
void MainWindow::startPlayMode(QString path, QString _titlePlaying)
{
    ...
    kbd.hide();
    if(PDF==nullptr || PDF->filepath != path)
    {
        deletePDF();
...
//    if(nowMode!=playMode || !playing) return;  // Only pay attention in playmode-playing
    if(nowMode==libraryMode) {
        libraryTable->keyPressEvent(e);
    }
    if(nowMode!=playMode) return;  // Only pay attention in playmode-playing
    if(playing){
        switch (e->key())
        {
        ...
        switch (e->key())
        {
            ...
            case Qt::Key_PageDown:
                qDebug() << "Received PageDown"; navigateTo(MUSICALPI_MAXPAGES); break;
            case Qt::Key_4:
            case Qt::Key_Left:
                qDebug() << "Received LeftArrow"; navigateTo(leftmostPage - (pagesNowAcross * pagesNowDown)); break;
            case Qt::Key_6:
            case Qt::Key_Right:
                qDebug() << "Received RightArrow"; navigateTo(leftmostPage + (pagesNowAcross * pagesNowDown)); break;
            case Qt::Key_9:
            case Qt::Key_PageUp:
                qDebug() << "Received PageUp";  navigateTo(1); break;
        }
    }
}
